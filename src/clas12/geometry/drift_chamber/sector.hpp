...
class Sector
{
  public:
    // inline methods
    const DriftChamber& drift_chamber() const;

    const vector<unique_ptr<Region>>& regions() const;
    const Region& region(const int& reg) const;
  ...
  private:
    Sector(const DriftChamber* drift_chamber, const size_t& idx);
    Sector(const Sector& that, const DriftChamber* drift_chamber, const size_t& idx);

    /// \brief a pointer to the parent sector
    const DriftChamber* _drift_chamber;
    ...
    size_t region_index(const int& idx) const;
 ...
 * \return constant reference to Sector::_drift_chamber
 **/
inline const DriftChamber& Sector::drift_chamber() const
{
    return *_drift_chamber;
}
 ...
 **/
inline const Region& Sector::region(const int& reg) const
{
 ...
 **/
inline
size_t Sector::region_index(const int& idx) const
{
